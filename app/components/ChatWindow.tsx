...
export function ChatWindow(props: {
  ...
  return (
    ...
    >
      <h2 className={`${messages.length > 0 ? "" : "hidden"} text-2xl p-2`}>
        {emoji} <b>{titleText}</b>
      </h2>
      {messages.length === 0 ? emptyStateComponent : ""}
      <div
        className="flex flex-col-reverse w-full mb-4 overflow-auto transition-[flex-grow] ease-in-out p-2"
      ...
      <form onSubmit={sendMessage} className="flex w-full flex-col">
        <div className="flex w-full mt-4">
          ...
          >
            ...
            <span className={`nowrap ${chatEndpointIsLoading ? "hidden" : ""}`}>
              Send&nbsp;&nbsp;▶
            </span>
          </button>
        </div>
